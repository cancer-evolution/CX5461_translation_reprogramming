###ClusterProfiler visualization of GeneGO results

library(ggplot2)

genego_up <- read.table("CMB_vs_CTRL_diff_translated (w bg list) - ALLSIG UPonly.txt", header=TRUE)
genego_down <- read.table("CMB_vs_CTRL_diff_translated (w bg list) - ALLSIG DOWN only.txt", header=TRUE)
genego_both <- read.table("CMB_vs_CTRL_diff_translated (w bg list) - ALLSIG both up and down.txt", header=TRUE)
